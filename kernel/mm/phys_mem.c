 ...
 * @brief 系统支持的最大物理页帧数
 *
 * @details QEMU virt 默认 1GB RAM，但内核静态管理表不应过大。
 *          8192 页 × 4KB = 32MB 可管理范围，足以覆盖内核 + 用户需求。
 *          剩余物理页可在 MMU 初始化后通过动态映射管理。
 *          调整此值可平衡 BSS 占用与可管理物理内存范围。
 *
 * @note 原 32768 (128MB) 导致 BSS 约 2MB，优化后节省 ~1.75MB
 */
#define MAX_PHYS_PAGES  8192U
