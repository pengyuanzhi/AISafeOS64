 ...
 * 简化系统调用号（基础接口层）
 *
 * @details 提供简化的系统调用编号，用于表驱动分发机制。
 *          这些编号独立于上述分类编号体系，用于快速查找。
 * ======================================================================== */
 ...
 * @details 返回调用者的线程标识符
 */
#define SYS_getpid                0U

/**
 * @brief 主动让出 CPU
 *
 * @details 触发调度器重新调度，允许同优先级或更高优先级线程运行
 */
#define SYS_yield                 1U
 ...
 * @brief 异步通知
 *
 * @details 向通知对象发送异步信号，不阻塞
 */
#define SYS_notify                5U

/**
 * @brief 内存映射
 *
 * @details 在指定地址空间中映射物理内存到虚拟地址
 */
#define SYS_map                   6U
 ...
 * @details 创建新的内核能力对象
 */
#define SYS_capability_create     8U

/**
 * @brief 派生能力
 *
 * @details 从现有能力派生子能力（可限制权限）
 */
#define SYS_capability_derive     9U
