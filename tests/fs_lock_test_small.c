...
static int32_t init_shards(void) {
    ...
    for (i = 0U; i < FS_SHARDS_COUNT; i++) {
        ...
        s_shard_locks[i] = (fs_lock_entry_t *)calloc(entries_per_shard, sizeof(fs_lock_entry_t));
...
static int32_t test_lock_operation(uint32_t mount_id, uint32_t ino,
    ...
    if (lock_type == 0x08U) {
        for (i = 0U; i < (FS_MAX_LOCKS / FS_SHARDS_COUNT); i++) {
            ...
            if (lock->in_use && lock->mount_id == mount_id && lock->ino == ino) {
...
static void *test_thread(void *arg) {
    ...
    for (i = 0U; i < TEST_ITERATIONS; i++) {
        ...
        int32_t ret = test_lock_operation(mount_id, ino, lock_type, tid);
        (void)ret;
        usleep(1000);
    }
