...
static int32_t init_shards(void) {
    ...
    for (i = 0U; i < FS_SHARDS_COUNT; i++) {
        ...
        shard_locks[i] = (fs_lock_entry_t *)calloc(FS_MAX_LOCKS / FS_SHARDS_COUNT,
...
static int32_t test_shard(uint32_t thread_id) {
    ...
    for (i = 0U; i < TEST_ITERATIONS; i++) {
        ...
        fs_lock_entry_t *lock = &s_lock_shards[shard_id].locks[base + (ino % entries_per_shard)];
