/**
 * @file    fs_lock_perf_no_memset.c
 * @brief   文件锁分片锁性能测试（无 memset）
...
static uint64_t g_lock_success = 0;
static uint64_t g_lock_failed = 0;
static uint64_t g_unlock_success = 0;
static uint64_t g_unlock_failed = 0;
...
static void *test_thread(void *arg) {
    ...
    for (i = 0U; i < TEST_ITERATIONS; i++) {
        ...
        fs_flock_lock(mount_id, ino, lock_type, tid);
        usleep(1000);
    }
...
int main(void) {
    ...
    printf("========================================\n");
    printf("文件锁分片锁性能测试（无 memset）\n");
    printf("========================================\n");
    printf("FS_MAX_LOCKS: %d, FS_SHARDS_COUNT: %d, TEST_THREADS: %d\n",
           FS_MAX_LOCKS, FS_SHARDS_COUNT, TEST_THREADS);
