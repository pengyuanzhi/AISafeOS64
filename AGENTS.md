...
## 强制开发规则：注释规范（Comment Standards）

**所有函数（公共 + 内部 static）必须有完整的 Doxygen 头注释。无注释的函数禁止提交。**
...
/**
 * @brief   一句话功能描述
 * @details 多行设计说明（为什么这样设计、实时性考量、并发策略）
 * @param   参数名  参数说明
 * @return  返回值说明
 * @note    特殊约束（如"禁止在中断中调用"）
 * @warning  警告（如"必须与 xxx 配对使用"）
 */
```
...
/**
 * @brief   一句话功能描述
 * @details 调用者必须持有什么锁/关调度/关中断等前置条件
 * @param   参数名  参数说明
 */
```
 ...
 * v2.0 YYYY-MM-DD 重大变更说明
 */
```

### 禁止事项

- ❌ 禁止无注释的函数（包括 static 内部函数）
- ❌ 禁止注释只描述"做了什么"而不描述"为什么"（设计决策必须有据可查）
- ❌ 禁止内联修订注释（如"P0-4 修复"、"FIXME"），修订记录放文件头 @revision history
- ❌ 禁止过时注释（代码改了注释没更新）

### 代码审查检查项

每次代码审查必须验证：
- [ ] 每个函数（含 static）都有 @brief
- [ ] 公共函数有 @param + @return + @note（如适用）
- [ ] 文件头有 @revision history
- [ ] 注释描述"为什么"而非仅"做了什么"
